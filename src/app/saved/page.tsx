  ...
  if (savedCalculators.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] gap-4 text-center">
        <Bookmark size={40} className="text-zinc-300" />
        ...
        <Link
          href="/categories"
          className="rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-indigo-700 transition-colors flex items-center gap-2"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {savedCalculators.map((slug) => {
          ...
          return (
            <div
              key={slug}
              className="group rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"
            >
              <div className="flex items-start justify-between gap-3">
                <Link href={`/${schema.category}/${schema.slug}`} className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-zinc-900 dark:text-white text-sm group-hover:text-indigo-600 transition-colors">
                      {schema.name}
                    </p>
                ...
                <button
                  ...
                  className="shrink-0 rounded-lg p-1.5 text-zinc-400 hover:text-red-500 hover:bg-red-50 transition-colors dark:hover:bg-red-950"
