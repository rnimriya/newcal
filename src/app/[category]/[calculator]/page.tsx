...
export default async function CalculatorPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Breadcrumb - Outside columns at page top */}
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-8">
          ...
          <section id="description" className="scroll-mt-20">
            <h2 className="section-title mb-3 font-bold text-xl ">What is the {entry.name}?</h2>
          ...
          <section id="how-to-use" className="scroll-mt-20">
            <h2 className="section-title mb-3 font-bold text-xl ">How to Use the {entry.name}</h2>
            <ol className="space-y-3">
              {content.howToSteps.map((step, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/20 text-base font-bold text-primary mt-0.5">
                    {i + 1}
                  </span>
          ...
          {dynamicContent.table && (
            <section id="reference-table-10" className="scroll-mt-20 space-y-4">
              <h2 className="section-title mb-3 font-bold text-xl ">Value Reference Table</h2>
                ...
                <table className="calc-table min-w-full">
                  ...
                  <tbody className="divide-y divide-border">
                    {dynamicContent.table.rows.map((row, i) => (
                      <tr key={i} className="hover:bg-muted odd:bg-card even:bg-secondary/50">
          ...
          <section id="practice-problems" className="scroll-mt-20 space-y-4">
            <h2 className="section-title mb-3 font-bold text-xl ">Practice Problems</h2>
            <p className="text-base text-muted-foreground">
            ...
            <div className="space-y-3">
              {dynamicContent.practiceProblems.map((prob, i) => (
                <div key={i} className="calc-card border border-border rounded-2xl  p-2 bg-card border border-border hover:border-primary/50 transition-colors rounded-2xl  p-5 bg-card border border-border hover:border-primary/50 transition-all rounded-2xl">
                  <div className="flex items-start gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/10 text-base font-bold text-primary mt-0.5">
                      Q{i + 1}
                    </span>
                    <div className="space-y-2.5 w-full">
                      ...
                      <details className="group">
                        ...
                        <div className="mt-3 text-base bg-secondary border border-border rounded-xl p-3.5 text-muted-foreground leading-relaxed font-medium">
                          {prob.answer}
                        </div>
                      </details>
                    </div>
          ...
          <section id="progression-chart" className="scroll-mt-20 space-y-4">
            <h2 className="section-title mb-3 font-bold text-xl ">Calculation Progression Roadmap</h2>
            <p className="text-base text-muted-foreground">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              {dynamicContent.progression.map((step, i) => (
                <div key={i} className="calc-card border border-border rounded-2xl  p-2 bg-card border border-border hover:border-primary/50 transition-colors rounded-2xl  p-4 bg-card border border-border relative flex flex-col justify-between hover:shadow-md transition-shadow rounded-2xl">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-black text-primary/25 tracking-wider">{step.step}</span>
                      {i < 4 && (
                        <span className="hidden md:inline text-primary/40 text-lg font-black absolute right-[-12px] top-1/2 -translate-y-1/2 z-10">
                          →
                        </span>
                      )}
                    </div>
          ...
          <section id="real-world-examples" className="scroll-mt-20 space-y-4">
            <h2 className="section-title mb-3 font-bold text-xl ">Real-World Examples</h2>
            <p className="text-base text-muted-foreground">
            ...
            <div className="space-y-3">
              {dynamicContent.powerExamples.map((ex, i) => (
                <div key={i} className="calc-card border border-border rounded-2xl  p-2 bg-card border border-border hover:border-primary/50 transition-colors rounded-2xl  p-5 bg-card border border-border hover:border-primary/50 transition-colors rounded-2xl">
                  <div className="flex items-start gap-3">
                    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-xl bg-emerald-500/10 text-base font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">
                      {i + 1}
                    </span>
                    <div>
          ...
          <section className="calc-card border border-border rounded-2xl  p-2 bg-card border border-border hover:border-primary/50 transition-colors rounded-2xl  p-5 bg-primary/5 border border-primary/20">
            ...
            <ul className="space-y-2 pb-2">
              {entry.formula && (
                <li className="text-base text-muted-foreground">
                  ...
                  <code className="bg-background px-2 py-0.5 rounded border border-border text-primary text-base">{entry.formula}</code>
                </li>
              )}
              <li className="text-base text-muted-foreground">
...
function StaticCalculatorCard({ entry }: { entry: ReturnType<typeof getEntry> }) {
  ...
  return (
    <div className="calc-card border border-border rounded-2xl  p-2 bg-card border border-border hover:border-primary/50 transition-colors rounded-2xl  p-6 space-y-4">
      ...
      {entry.formula && (
        <div>
          <p className="text-base font-semibold uppercase tracking-widest text-muted-foreground mb-2">Formula</p>
